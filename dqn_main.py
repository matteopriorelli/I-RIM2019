...
def run_episode(episode, dqn):
    ...
    while not episode_done and step < param.steps:
        ...
        prediction_q_actions = dqn.feedforward(state)
        ...
        # update weights
        loss = dhl.update_network(dqn, state, next_state, reward)
...
def main():
    # initialize agents
    dqn = Agent(param.alpha, param.n_observations, param.n_actions, param.n_neurons)
